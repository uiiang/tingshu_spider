...
def req_ting_serz(code):
  response = requests.get(f'http://yuetingba.cn/api/app/docs-listen/{code}/ting-serz', headers)
  # print(response.text)
...
def check_not_exists_file(dir, download_list_json):
  ...
  file_list = list(map(lambda f: remove_ext(f), file_list))
  return list(filter(lambda f: not f['title'] in file_list, download_list_json))
  
def save_download_list_json(download_json):
  ...
  with open(os.path.join(title, 'download_list.json'), 'w',encoding='utf-8') as f:
    f.write(b)
    f.close()
def get_download_list(chapter_list):
  download_list = list()
  for cht in chapter_list:
    # if not check_file_exists(cht[1]):
    b64 = req_ting_serz(cht[0])
    json_str = decode_chapter_json(b64)
...
def download_book(src, dir_path, file_name):
...
def download_chapter(download_list):
  lock = Lock()
  dir_path = title
  ...
  for index, down_info in enumerate(download_list):
    ...
    time.sleep(1)
    lock.acquire()
    t = Thread(target=download_book, args=(down_info['filePath'], title, chapter_title + ext),
...
if __name__ == "__main__":
  book_id,start_chapter,end_chapter,cov_to_mp3 = input_info()
  driver = get_driver()
  time.sleep(1)
  open_url()
  time.sleep(1)
  soup,title = get_book_title()
  ...
  if os.path.exists(os.path.join(title, 'download_list.json')):
    ...
    download_list_json = get_download_list_json()
    download_list = check_not_exists_file(os.path.join(title,'download'),download_list_json)
    print(f'download_list中保存{len(download_list_json)}个章节信息，本地缺少{len(download_list)}个文件')
    # print(download_list)
    download_chapter(download_list)
    ...
    download_list = get_download_list(temp_list)
    print(f'有声读物《{title}》共找到 {len(download_list)} 个章节的下载地址')
    save_download_list_json(download_list)
    download_chapter(download_list)
